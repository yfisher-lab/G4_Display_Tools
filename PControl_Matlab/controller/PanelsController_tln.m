...
classdef PanelsController < handle
    ...
    methods 
        ...
        function rtn = setAO(self, aoChannel, voltage)
            ...
            rtn = false;
            chnl = aoChannel - 5;   % maps AO6→1, AO7→2
            volVar = int16(voltage / 10 * intmax('int16'));  % scale to int16
        
            if voltage < 0
                cmdData = char([4 17]);   % 0x04 0x11 — negative voltage
                volAbs = dec2char(uint16(abs(volVar)), 2);
                self.write([cmdData dec2char(chnl, 1) volAbs]);
            else
                cmdData = char([4 16]);   % 0x04 0x10 — positive voltage
                volAbs = dec2char(uint16(volVar), 2);
                self.write([cmdData dec2char(chnl, 1) volAbs]);
            end
